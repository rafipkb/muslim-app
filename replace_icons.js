...
for (const [lucide, iconId] of Object.entries(subs)) {
    const regex = new RegExp(`<i data-lucide="${lucide}"></i>`, 'g');
    if (lucide === 'sun') {
        content = content.replace(regex, `<svg class="icon" id="theme-icon"><use href="#${iconId}"></use></svg>`);
    } else {
        content = content.replace(regex, `<svg class="icon"><use href="#${iconId}"></use></svg>`);
    }
}

// Special case for Qibla pointer
content = content.replace(
    /<i data-lucide="arrow-up" id="qibla-pointer" class="kaaba-pointer"><\/i>/g,
    `<svg id="qibla-pointer" class="kaaba-pointer icon"><use href="#icon-kaaba"></use></svg>`
);

// Remove lucide script
content = content.replace(/<script src="https:\/\/unpkg\.com\/lucide@latest"><\/script>\s*/g, '');

const svgDefs = `<svg width="0" height="0" class="hidden">
  <defs>
